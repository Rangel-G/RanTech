...
export interface LedSettings {
  name: string;
  uuid: string;
  deviceId?: string; // ID único/MAC do dispositivo BLE para vínculo permanente
  redlineRpm: number;
  blinkSpeed: number;
  colorNormal: string;
  colorRedline: string;
...
export const SettingsStorage = {
  async getObdSettings(): Promise<ObdSettings> {
    try {
      ...
      return data
        ? { ...DEFAULT_OBD_SETTINGS, ...JSON.parse(data) }
  ...
  async getLedSettings(): Promise<LedSettings> {
    try {
      ...
      return data
        ? { ...DEFAULT_LED_SETTINGS, ...JSON.parse(data) }
  ...
  async getGearSettings(): Promise<GearSettings> {
    try {
      ...
      return data
        ? { ...DEFAULT_GEAR_SETTINGS, ...JSON.parse(data) }
